...
if (SessionUtilities::checkIsSessionSet('User')) {
    if (!SessionUtilities::checkIsSessionSet('EditPPAFilter'))
        SessionUtilities::createSession('EditPPAFilter', '');
    if (isset($_POST['chosenBarcode']))
        SessionUtilities::createSession('EditPPAFilter', 'barcodeID');
    else if (isset($_POST['chosenFabricId']))
        SessionUtilities::createSession('EditPPAFilter', 'fabricId');
...
function fetchDevicesFromDB() {
    ...
    if (SessionUtilities::getSession('EditPPAFilter') === 'fabricId')
        $additionalFields = ', P.fabricId';

    $sql = "SELECT S.id, B.allowedBarcodes as barcodeID" . $additionalFields . "
            FROM sviuredjaji S, barcodes B, ppaparati P
            WHERE S.barcodeID=B.id AND P.sviUredjajiId=S.id AND S.type NOT IN ('UPP', 'Hydrants')  
...
function printDevices($devices) {
    foreach ($devices as $device) {
        if (SessionUtilities::getSession('EditPPAFilter') === 'fabricId')
            echo "<div class=\"filter-option\" id=\"" . $device['id'] . "\" onclick=\"sendChosenPPA(this.id)\" data-type-id=\"".$device['id']."\" data-search-term=\"" . $device['fabricId'] . "\"> <span class=\"option-id\">".$device['id']."</span> ".$device['fabricId']."</div>";
        else
            echo "<div class=\"filter-option\" id=\"" . $device['id'] . "\" onclick=\"sendChosenPPA(this.id)\" data-type-id=\"".$device['id']."\" data-search-term=\"" . $device['barcodeID'] . "\"> <span class=\"option-id\">".$device['id']."</span> ".$device['barcodeID']."</div>";
    }
}
...
<head>
    ...
    <title>PBS - Podaci o PP aparatima</title>
    ...
    <div class="container">
        ...
        <div class="filter-grid">
            <div class="types-page">
                ...
                <div class="actions">
                    <form action="edit_ppas.php" method="post">
                        <input name="chosenBarcode" class="buttons default-button" type="submit" value="Barkod">
                        <input name="chosenFabricId" class="buttons default-button" type="submit" value="Fabricki broj">
                    </form>
                </div>
                <div class="filter-body">
                    ...
                    <div class="devices-wrapper ppas">
