...
if (SessionUtilities::checkIsSessionSet('User')) {
    if (!SessionUtilities::checkIsSessionSet('EditHydrantsFilter'))
        SessionUtilities::createSession('EditHydrantsFilter', '');
    if (isset($_POST['chosenBarcode']))
        SessionUtilities::createSession('EditHydrantsFilter', 'barcodeID');
...
function fetchDevicesFromDB() {
    ...
    $sql = "SELECT S.id, B.allowedBarcodes as barcodeID" . $additionalFields . "
            FROM sviuredjaji S, barcodes B, hidranti H
            WHERE S.barcodeID=B.id AND H.sviUredjajiId=S.id AND S.type='Hydrants' 
...
function printDevices($devices) {
    foreach ($devices as $device) {
        echo "<div class=\"filter-option\" id=\"" . $device['id'] . "\" onclick=\"sendChosenHydrant(this.id)\" data-type-id=\"".$device['id']."\" data-search-term=\"" . $device['barcodeID'] . "\"> <span class=\"option-id\">".$device['id']."</span> ".$device['barcodeID']."</div>";
    }
}
...
<head>
    ...
    <title>PBS - Podaci o Hidrantima</title>
    ...
    <div class="container">
        ...
        <div class="filter-grid">
            <div class="types-page">
                ...
                <div class="actions">
                    <form action="edit_hydrants.php" method="post">
